...
本工程由 vite-tpl 的 PC 模板生成，使用 Ant Design Vue，采用普通桌面布局。
...
运行环境：Node.js 20.19.0+，pnpm 9.15.9。依赖安装由创建技能默认执行。
...
├── ui.config.ts                     # Ant Design Vue 组件解析器
...
UI 组件解析器在 `ui.config.ts` 中维护，共享的 `vite.config.ts` 引用它：
...
```ts
// vite.config.ts 中的 Components 配置
Components({
    dirs: ['src/components'],
    deep: false,
    ...
    resolvers: [uiResolver], // 同时保留项目的图标和 VueUse 解析器
});
```

- `dirs` 指定查找目录，组件不是全局注册；子目录组件需要显式导入。
- 测试文件不会被当作组件扫描。
- `src/types/components.d.ts` 由插件自动生成，不得手动修改。
